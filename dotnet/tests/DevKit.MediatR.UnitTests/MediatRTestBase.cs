...
public class MediatRTestBase
{
    ...
    public ICommandHandler<TestCommand, object> CommandHandler { get; }

    public MediatRTestBase()
    {
        QueryHandler = Substitute.For<IQueryHandler<TestQuery, object>>();
        CommandHandler = Substitute.For<ICommandHandler<TestCommand, object>>();

        var services = new ServiceCollection()
            .AddSingleton<IRequestHandler<TestQuery, object>>(_ => QueryHandler)
            .AddSingleton<IRequestHandler<TestCommand, object>>(_ => CommandHandler)
